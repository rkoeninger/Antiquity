...
typedef vector<StudentData> StudentList;
...
void sort(StudentList&, bool); // bool is true for accending,
                                       // false for decending
void sortSubArray(StudentList&, bool, int, int);
void mergeSubArrays(StudentList&, bool, int, int, int, int);
...
void showAll(StudentList);
void showClass(StudentList, int, char*);

int main()
{

    // Open streams to in and out files
    ifstream *scoresIn = new ifstream("scores17.txt");
    ofstream *fileOut = new ofstream("proj17out.txt");
    fout = fileOut;

    // A growable vector of student data entries
    StudentList dataList(1);
    ...
    // Bubble-sort by average (accending)
    for (int x = 0; x < dataList.size(); ++x)
        for (int y = 0; y < dataList.size(); ++y)
            if (dataList[y].average < dataList[y - 1].average)
            {

                StudentData temp = dataList[y];
...
void sort(StudentList &array, bool accending)
{
...
void sortSubArray(StudentList &array, bool accending, int begin, int end)
{
...
void mergeSubArrays(StudentList &array, bool accending,
...
double modeGender(StudentList dataList, int gender)
{
...
void showAll(StudentList dataList)
{
...
void showClass(StudentList dataList, int year, char* yearLabel)
{
