;******************************************************************************
;                                                                             *
;   Filename:                stopwatch.asm                                    *
;   Date:                    Monday, June 8, 2009                             *
;   Author:                  Robert Koeninger                                 *
;   Class:                   Microsystem Design and Programming               *
;                            Spring Quarter 2009                              *
;                                                                             *
;******************************************************************************
...
; Program Body
; The main body starts by initializing the input and output pins, then moves to
; the waiting state. In the waiting state, it loops indefinitely, flashing a
; pattern on the LEDs. When the button is pushed, it moves to the running state
; where it counts off time in 10ms increments and rotates an LED every second.
; When the button is pushed again, it moves to the display state, where it
; shows a blank display for 1 sec, shows the ten's and one's place as BCDs for
; 5 sec, blank again for 1 sec, shows the tenth's and hundredth's place as BCDs
; for 5 sec, blank again for 1 sec and then returns to the waiting state.
    org 0
Init:
...
TimerWait:
    movlw   0x55
    movwf   PORTD            ; Initialize timer display for waiting state
TimerWaitingLoop:
...
DisplayTime:
    ...
    swapf   TimeTens,f       ; Swap the ten's place 4 bits over and...
    movfw   TimeTens
    ...
    clrf    PORTD            ; Clear display
    call    Wait1s           ; Show blank display for 1 sec
    swapf   TimeTenths,f     ; Swap the tenth's place 4 bits over and...
    movfw   TimeTenths
